...
![badges](https://img.shields.io/badge/Author-AndrewYq-orange.svg)
...
统计学习的目的是使学到的模型不仅对于已知数据而且对未知数据都能有很好的预测能力。不同的学习方法给出不同的模型。当损失函数给定时，基于损失函数的模型的训练误差（training error）和模型的测试误差（test error）就自然成为学习方法评估的标准。统计学习方法采用的损失函数未必是评估时使用的损失函数。

假设学习到的模型是![model](https://latex.codecogs.com/gif.latex?Y%20%3D%20%5Chat%20f%28X%29),训练误差是模型![model](https://latex.codecogs.com/gif.latex?Y%20%3D%20%5Chat%20f%28X%29)关于训练数据集的平均损失：
...
测试误差是模型![model](https://latex.codecogs.com/gif.latex?Y%20%3D%20%5Chat%20f%28X%29)关于测试集数据的平均损失：
